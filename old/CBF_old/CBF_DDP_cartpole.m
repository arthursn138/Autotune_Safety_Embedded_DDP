...
% Mods by Arthur Scaquetti do Nascimento: June 2022
...
%% Discrete DDP
[X,U,J,lambda,dlambda,alpha,k_u,K_u] = disc_ddp_alg(0,f_dyn,run_cost,...
    term_cost,sys_par,ubar,xbar,opt_par);
...
%% plotting ----- ARTHUR: CHANGED LEGEND ORDER TO MATCH CORRECT OUTPUTS
figure(1)
...
legend('Wall','Wall','DBaS-DDP','Unconstrained DDP','FontName','Times New Roman');
box on;
figure(2)
plot(T,xf(2)*ones(length(T),1),'-g','LineWidth',1.5); hold on;
plot(T,X_sim(2,:),':','LineWidth',1.5); 
plot(T,X(2,:),'--','LineWidth',1.5);
title('Pendulum angle vs time');
ylabel('$\theta$ (rad)','FontName','Times New Roman','Interpreter','latex');
xlabel('Time (s)','FontName','Times New Roman');
legend('Target angle','DBaS-DDP','Unconstrained DDP','FontName','Times New Roman');
% legend('Targe angle','DBaS-DDP','Unconstrained DDP','FontName','Times New Roman');
box on;
figure(3)
plot(0:dt:dt*N-2*dt,U_sim,':','LineWidth',1.5); hold on;
plot(0:dt:dt*N-2*dt,U,'--','LineWidth',1.5);
title('Control effort vs time');
ylabel('$\theta$ (rad)','FontName','Times New Roman','Interpreter','latex');
xlabel('Time (s)','FontName','Times New Roman');
legend('DBaS-DDP','Unconstrained DDP');