...
% This is an example for a differential-wheeled robot. 
% As of March 16 we are completely ignoring every safety aspect (no BaS
% whatsoever -> skip steps 2, 3 and 4.
...
% opt_par = struct('iter',iter,'toler',toler); % For no line searching or regularization
...
plot(dt*N,xf(1),'*r','LineWidth',1.5)
...
plot(dt*N,xf(2),'*r','LineWidth',1.5)
title('Y position vs time','Interpreter','latex');
ylabel('$y$ (m)','FontName','Times New Roman','Fontsize',12,'Interpreter','latex');
xlabel('Time (s)','FontName','Times New Roman','Fontsize',12,'Interpreter','latex');
% box on;
subplot(2,2,3)
plot(T,X(3,:),'LineWidth',1.5); hold on; grid on;
plot(T,xf(3)*ones(length(T)),'LineWidth',1.2)
...
ylabel('$u$','FontName','Times New Roman','Fontsize',12,'Interpreter','latex');
xlabel('Time (s)','FontName','Times New Roman','Fontsize',12,'Interpreter','latex');
box on;
