...
% This is an example for 2D differential wheeled robot with Barrier States
% and control-limited discrete DDP.
...
dt = 0.02;  % Sampling
N = 300;    % Horizon (# of iteration that discretizes each pass)
% T = dt*N;
T = 0:dt:dt*N-1*dt;
...
% Q(2,2) = 0.5;     % States weights
% Q(1,1) = 0.5;     % States weights

R = 0.5e-2*eye(m);  % Controls weights
S = 500*eye(n);     % Final cost
% S(3,3) = 1;
Q_bf = 1e-4;
S_bf = 0.05;
...
% Input constraints
u_lims  = [-1.5 3.0;      % Left(?) wheel controller (in rad/s? Nm?)
             -1.5 3.0];   % Right(?) wheel controller (in rad/s? Nm?)
...
[X,U,J,lambda,dlambda,alpha,k_u,K_u,ii,iter_succ,L] = safe_ctrl_lim_disc_ddp_alg(ddp_2nd_order,...
...
%% Plottings
car_2d_plot(T,X,U,x0,xf,u_lims,obstacles);
