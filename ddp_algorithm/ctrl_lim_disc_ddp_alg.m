function [x, u,J,lambda,dlambda,alpha,k_u,K_u,ii,iter_succ,total_cost] = ctrl_lim_disc_ddp_alg(ddp_2nd_order,f_dyn,run_cost,term_cost,ddp_par,ubar,xbar,u_lims,opt_par,h);
...
% % %             K_u(:,:,jj) = -Quu_reg\Qux;  % fb gain
            ...
            % For input constraints: box constraint QP by Tassa, Y. and Mansard, N. and Todorov, E.
            % Added by Arthur Nascimento
            if isempty(u_lims) || u_lims(1,1) > u_lims(1,2)
                ...
                % TRY THE FUNCTION QUADPROG - COULD THIS REPLACE
                % boxQP.m???? https://www.youtube.com/watch?v=2LbNIDwla_c
                % https://www.youtube.com/watch?v=GZb9647X8sg
                % Tassa proposes PROJECTED-NEWTON, which has line
                % searching, so by ditching boxQP.m we'd be losing that
                % line searching --- implement that bymyself
            ...
            % Note: I think we are wasting comp power with the tensors k
            % and K: we should need just one of each per iteration, thus
            % k_u(m) and K_u(m,n) would suffice --- Check how Yuval does
            % it and try to replicate
