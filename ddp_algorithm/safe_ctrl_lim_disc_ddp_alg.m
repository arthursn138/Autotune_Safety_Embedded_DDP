function [x,u,J,lambda,dlambda,alpha,k_u,k_u_all,K_u,ii,iter_succ,total_cost] = safe_ctrl_lim_disc_ddp_alg(ddp_2nd_order,...
    f_dyn,run_cost,term_cost,ddp_par,ubar,xbar,u_lims,opt_par,h,hx,alpha_cbf);
...
% % %             end
            ...
            if isempty(u_lims) || u_lims(1,1) > u_lims(1,2)
                k_u(:,jj) = -Quu_reg\Qu;    % ff gain
                K_u(:,:,jj) = -Quu_reg\Qux; % fb gain
            else
                
                % Solve Quadratic Programming for optimal OL gain:
                % CBF-Aware DDP

                hx_cbf = hx{1}(xbar(1:2,jj));
                
                ku = k_u(:,jj);
                fminimize = @(ku) ku'*Quu_reg*ku + Qu'*ku; 
% %                 fminimize = @(k_u) (k_u - ku)'*(k_u - ku);
                k_u(:,jj) = fmincon(fminimize, k_u(:,jj), [], [], [], [], [], [],...
                    @(k_u) constraint_cbf(k_u,hx_cbf,f_dyn.f,f_dyn.g,alpha_cbf,xbar(:,jj)));
...
% % %                 A = -(hx_cbf*f_dyn.g);
% % %                 b = hx_cbf*f_dyn.f + alpha_cbf(xbar(:,jj));
% % %                 k_u(:,jj) = quadprog(Quu_reg,Qu,A,b,[],[],[],[],ku,options);
                
                k_u_all = [k_u_all; k_u];

% % % % % %                 % For using box constraint QP by Tassa et al
% % % % % %                 lower = u_lims(:,1)-ubar(:,jj);
% % % % % %                 upper = u_lims(:,2)-ubar(:,jj);
% % % % % % 
% % % % % %                 lower = min(lower,min(ku(1),ku(2)));
% % % % % %                 
% % % % % %                 % Calculating OL feedback gain k:
% % % % % %                 [k_u(:,jj),~,R,free] = boxQP(Quu_reg,Qu,lower,upper,k_u(:,jj));
% % % % % %                 % R is the Cholesky factorized Quu_reg matrix
% % % % % %                 % Calculating CL feedback gain K:
% % % % % %                 if any(free)
% % % % % %                     Lfree = -R\(R'\Qux(free,:));
% % % % % %                     K_u(free,:,jj) = Lfree;
% % % % % %                 end
...
%         alpha0=1;
        for alpha = alpha0 
            ...
            for jj=1:N -1
                ...
                hx_cbf = hx{1}(xbar(1:2,jj));
                ku = u(:,jj);
                fminimize = @(u) (u - ku)'*(u - ku);
                u(:,jj) = fmincon(fminimize, ku, [], [], [], [], [], [],...
                    @(u) constraint_cbf(u,hx_cbf,f_dyn.f,f_dyn.g,alpha_cbf,xbar(:,jj)));
                
                % For input box constraints, do a Naive Clamping to make
                % sure we overwrite any bad controls from the backward pass
                % by Arthur Nascimento
% % %                 if ~isempty(u_lims)
% % %                     u(:,jj) = min(u_lims(:,2), max(u_lims(:,1), u(:,jj)));
% % %                 end
...
% % % % % % % % % % % % % %                 % Penalizes the exterior of safe sets
% % % % % % % % % % % % % %                 for kk=1:length(h)
% % % % % % % % % % % % % %                     if h{kk}(x(:,jj + 1)) < 1e-15
% % % % % % % % % % % % % %                     L = L+1e10;
% % % % % % % % % % % % % %                     break;
% % % % % % % % % % % % % %                     end
% % % % % % % % % % % % % %                 end    
