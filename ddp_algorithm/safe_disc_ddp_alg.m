function [x,u,J,lambda,dlambda,alpha,k_u,k_u_all,K_u,ii,iter_succ,...
    total_cost,safety,w_back,w_fwd] = safe_disc_ddp_alg(ddp_2nd_order,...
    f_dyn,run_cost,term_cost,ddp_par,ubar,xbar,opt_par,h,hx,obs_loc)

% First order discrete DDP (iLQR) with limit constraints and penalty for
% CBF
% Regularization part is by Yuichiro Aoyama
% Safety part is by Arthur Nascimento
...
w_back = zeros(1,N); w_fwd = zeros(1,N-1);
...
total_cost = L + L_f;
% initialize the costs for each iteration
J = zeros(1, iter);
...
for ii = 1:iter
    ...
    for jj=N-1:-1:1
        ...
        % Embedded safety: input constraints and CBF awareness (by
        % Arthur Nascimento)hx_cbf = hx{1}(xbar(1:2,jj));

        % Solve Quadratic Programming for optimal OL gain (best k_u):
        h_cbf = h{1}(xbar(1:2,jj));
        hx_cbf = hx{1}(xbar(1:2,jj));

% % %         % **WITHOUT** optimal decay
% % %         ku = k_u(:,jj);
% % %         fminimize = @(ku) ku'*Quu_reg*ku + Qu'*ku; % NOOO: IT'S QX!!!
% % %         k_u(:,jj) = fmincon(fminimize, k_u(:,jj), [], [], [], [], [], [],...
% % %             @(k_u) constraint_cbf(k_u, hx_cbf, f_dyn.f, f_dyn.g,...
% % %             alpha_cbf, xbar(:,jj)), fmincon_options);

        ku = [k_u(:,jj); w_back(:,jj)];

        % USING OPTIMAL DECAY + MIN NORM
        k_uu(:,jj) = fmincon(@(k_u) minnorm(k_u, ku), ku, [], [], [], [], [], [],...
            @(k_u) constraint_cbf(k_u, h_cbf, hx_cbf, f_dyn.f, f_dyn.g), fmincon_options);
        
% % % % % % %         % USING OPTIMAL DECAY + TASSA'S OBJECTIVE FUNCTION
% % % % % % %         k_uu(:,jj) = fmincon(@(k_u) Tassa(ku, Quu_reg, Qx), ku, [], [], [], [], [], [],...
% % % % % % %             @(k_u) constraint_cbf(k_u, h_cbf, hx_cbf, f_dyn.f, f_dyn.g), fmincon_options);
        
        k_u(1:2,jj) = k_uu(1:2,jj);
        w_back(jj) = k_uu(3,jj);
    ...
    % FORWARD PASS: + 20*norm(u(3))
    fwdPassDone =0;
    % initilizations
    x = zeros(n,N); safety = zeros(1,N);
    x(:,1) = x0;
    u = zeros(m,N-1);
    deltax = x(:,1) - xbar(:,1);
    ...
    for alpha = alpha0
        ...
        for jj = 1:N -1
            ...
            % CBF-Aware DDP in the FORWARD propagation
            h_cbf = h{1}(xbar(1:2,jj));
            hx_cbf = hx{1}(xbar(1:2,jj));

% % % % %             % ***WITHOUT*** optimal decay
% % % % %             ku = u(:,jj);
% % % % %             fminimize = @(u) (u - ku)'*(f_dyn.gu - ku);
% % % % %             u(:,jj) = fmincon(fminimize, ku, [], [], [], [], [], [],...
% % % % %                 @(u) constraint_cbf(u, hx_cbf, f_dyn.f, f_dyn.g,...
% % % % %                 alpha_cbf, xbar(:,jj)), fmincon_options);alpha_cbf = @(x) alpha_h*h{1}(x);

            % WITH optimal decay
            ku = [u(:,jj); 0];
            uu(:,jj) = fmincon(@(u) minnorm(u, ku), ku, [], [], [], [], [], [],...
                @(u) constraint_cbf(u, h_cbf, hx_cbf, f_dyn.f, f_dyn.g), fmincon_options);
            u(1:2,jj) = uu(1:2,jj);
            w_fwd(jj) = uu(3,jj);
...
% Check for collisions --- ASSUMES A CIRCLE
for k = 1:length(x)
    safety(k) = (obs_loc(1) - x(1,k))^2 + (obs_loc(2) - x(2,k))^2;
end

if min(safety(1:end)) <= obs_loc(3)^2
    safety = 1;
else
    safety = 0;
end
...
% CBF Objective Functions --- Only works for relative degree 1

% % % function f = minnorm(u, ku)
% % % f = (u(1:2) - ku(1:2))'*(u(1:2) - ku(1:2));
% % % end

% For Optimal Decay
function f = Tassa(ku, Quu, Qx)
f = ku(1:2)'*Quu*ku(1:2) + Qx'*ku(1:2) + 0*1e9*ku(3)^2;
end


function f = minnorm(u, ku)
f = (u(1:2) - ku(1:2))'*(u(1:2) - ku(1:2)) + 0.05*u(3)^2;
end

% 0.005*u(3)^2 worked pretty well for x0 = (0,0), xf = (1,1), obs_loc =
% (0.4, 0.5, 0.1) --- ALL MIN NORM, NO TASSA YET

% % % % CBF Constraint
% % % function [c, ceq] = constraint_cbf(k_u,hx_cbf,f,g,alpha_cbf,x)
% % % c = -(hx_cbf*(f + g*k_u) + alpha_cbf(x));
% % % % c = -(hx_cbf*f(x,ku) + alpha_cbf(x));
% % % ceq = [];
% % % end

% CBF Constraint using Optimal Decay
function [c, ceq] = constraint_cbf(k_u,h_cbf,hx_cbf,f,g)
c = -(hx_cbf(1:2)*(f(1:2) + g(1:2)*k_u(1:2)) + k_u(3)*h_cbf);
ceq = [];
end
